...
const Navbar = () => {
  ...
  // Use React Router to get the current pathname
  const location = useLocation();
  const hideNavLinksOn = ["/profile", "/dashboard", "/my-learning", "/login"];
  const shouldHideLinks = hideNavLinksOn.includes(location.pathname);

  // ScrollLink common props
  const commonProps = {
    smooth: true,
    duration: 300,
    offset: -64,
        ...
        {!shouldHideLinks && (
          <nav className="flex items-center justify-center gap-6 text-xl font-medium text-black dark:text-gray-300">
            <ScrollLink
              to="home"
              {...commonProps}
              className={`relative group cursor-pointer ${active === "home" ? "active-link" : ""}`}
            >
            ...
            <ScrollLink
              to="courses"
              {...commonProps}
              className={`relative group cursor-pointer ${active === "courses" ? "active-link" : ""}`}
            >
            ...
            <ScrollLink
              to="about"
              {...commonProps}
              className={`relative group cursor-pointer ${active === "about" ? "active-link" : ""}`}
            >
            ...
            <ScrollLink
              to="recommendation"
              {...commonProps}
              className={`relative group cursor-pointer ${active === "recommendation" ? "active-link" : ""}`}
            >
